...
int32 arm_inst_qadd8(cpu_state_t *st, uint32 inst)
{
	...
	//低位结果
	int64 resLL = bitm(regvl(rm), 0, 7) + bitm(regvl(rn), 0, 7);
	uint32 LL = signed_sat(resLL, int_min(8), int_max(8), &issat);
	int64 resLH = bitm(regvl(rm), 8, 15) + bitm(regvl(rn), 8, 15);
	uint32 LH = signed_sat(resLH, int_min(8), int_max(8), &issat);
	//高位结果
	int64 resHL = bitm(regvl(rm), 16, 23) + bitm(regvl(rn), 16, 23);
	uint32 HL = signed_sat(resHL, int_min(8), int_max(8), &issat);
	int64 resHH = bitm(regvl(rm), 24, 31) + bitm(regvl(rn), 24, 31);
